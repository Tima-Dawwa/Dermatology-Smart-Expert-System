...
class ModernFreshDermatologyGUI:
    def __init__(self, engine_class):
        ...
        self.choice_radio_var = tk.StringVar()
    ...
    def setup_modern_styles(self):
        ...
        style.map("ModernSecondary.TButton", background=[("active", "#059669")])
        ...
        style.map(
            "ModernGhost.TButton", background=[("active", self.colors["gray_200"])]
        )
        ...
        style.configure(
            "ModernHeading.TLabel",
            background=self.colors["bg_card"],
            foreground=self.colors["text_primary"],
            font=("Segoe UI", 16, "bold"),
        )
        style.configure(
            "ModernSubheading.TLabel",
            background=self.colors["bg_card"],
        ...
        style.configure(
            "ModernSuccess.TLabel",
            background=self.colors["bg_primary"],
            foreground=self.colors["success"],
            font=("Segoe UI", 9, "bold"),
        )
        style.configure(
            "ModernWarning.TLabel",
            background=self.colors["bg_primary"],
    ...
    def create_main_interface(self):
        ...
        shadow_frame = tk.Frame(main_container, bg=self.colors["gray_300"], height=2)
    ...
    def create_modern_header(self, parent):
        ...
        title_label = tk.Label(
            title_frame,
            text="Dermatology Expert System",
    ...
    def create_modern_content_area(self, parent):
        ...
        self.notebook.grid(row=1, column=0, sticky="nsew", padx=20, pady=(5, 5))

        self.diagnosis_frame = ttk.Frame(self.notebook, style="ModernCard.TFrame")
    ...
    def create_modern_diagnosis_tab(self):
        ...
        self.welcome_frame = tk.Frame(self.diagnosis_frame, bg=self.colors["bg_card"])
        self.welcome_frame.grid(row=0, column=0, sticky="ew", padx=20, pady=(15, 10))
        ...
        welcome_header = tk.Label(
            welcome_inner,
            text="🌟 Welcome to Your Personal Dermatology Assistant",
            bg="white",
            fg=self.colors["text_primary"],
        ...
        self.question_frame = tk.Frame(self.diagnosis_frame, bg=self.colors["bg_card"])
        self.question_frame.grid(row=1, column=0, sticky="nsew", padx=20, pady=5)
        self.question_frame.grid_rowconfigure(1, weight=1)
        self.question_frame.grid_columnconfigure(0, weight=1)

        question_card = tk.Frame(self.question_frame, bg="white", relief="flat", bd=0)
        ...
        question_header = tk.Frame(question_card, bg=self.colors["primary"], height=40)
        ...
        progress_section = tk.Frame(self.diagnosis_frame, bg=self.colors["bg_card"])
        progress_section.grid(row=2, column=0, sticky="ew", padx=20, pady=(5, 10))

        progress_card = tk.Frame(progress_section, bg="white", relief="flat", bd=0)
        ...
        self.results_frame = tk.Frame(self.diagnosis_frame, bg=self.colors["bg_card"])
        self.results_frame.grid(row=3, column=0, sticky="ew", padx=20, pady=(5, 20))
        self.results_frame.grid_columnconfigure(0, weight=1)

        results_card = tk.Frame(self.results_frame, bg="white", relief="flat", bd=0)
        results_card.grid(row=0, column=0, sticky="ew", padx=3, pady=3)
        results_card.grid_columnconfigure(0, weight=1)

        results_header = tk.Frame(results_card, bg=self.colors["secondary"], height=40)
    ...
    def process_engine_state(self):
        ...
        next_question_fact = next(
            (
                f
                for f in self.expert_system.facts.values()
        ...
        if next_question_fact:
            ...
            messagebox.showinfo(
                "Diagnosis Complete",
                "The expert system has finished running, but no explicit diagnosis was found or question asked. This might indicate incomplete rules or a lack of relevant information.",
            )
    ...
    def handle_question(self, question_ident):
        try:
            ...
            if not question_data:
                raise ValueError(f"Question with ident '{question_ident}' not found.")
    ...
    def display_question(self):
        ...
        self.choice_radio_var.set("")
        self.checkbox_vars_dict = {}

        if question_type == "number":
            print(f"DEBUG_DISPLAY: Creating NUMBER input for '{question_ident}'")
            ...
            messagebox.showerror(
                "System Error",
                f"Unknown question type: {question_type} for '{question_ident}'",
            )
    ...
    def create_modern_number_input(self):
        ...
        self.number_entry = ttk.Entry(
            input_container,
            textvariable=self.answer_text_var,
            font=("Segoe UI", 11),
            width=30,
            style="Modern.TEntry",
        )
        ...
        submit_button = ttk.Button(
            ...
            text="Submit Answer",
            style="ModernPrimary.TButton",
            command=self.submit_number_answer,
        )
        submit_button.pack(anchor="w")
        self.number_entry.bind("<Return>", lambda e: self.submit_number_answer())

    def create_modern_multiple_choice_input(
        self, valid_responses, allow_multiple=False
    ):
        input_container = tk.Frame(self.answer_frame, bg="white")
        ...
        canvas.create_window(
            (0, 0), window=button_frame_inner, anchor="nw", width=canvas.winfo_width()
        )

        def on_canvas_configure(event):
            canvas.itemconfig(
                canvas.find_withtag("button_frame_inner_id"), width=event.width
            )
        ...
        if allow_multiple:
            ...
            for i, option in enumerate(valid_responses):
                ...
                var = tk.BooleanVar()
                self.checkbox_vars_dict[option] = var
                ...
                checkbox.grid(row=row_idx, column=0, sticky="w", pady=3, padx=8)
                row_idx += 1
        else:
            row_idx = 0
            for i, option in enumerate(valid_responses):
                ...
                radio_button = ttk.Radiobutton(
                    button_frame_inner,
                    text=display_text,
                    variable=self.choice_radio_var,
                    value=option,
                    style="Modern.TRadiobutton",
                )
                radio_button.grid(row=row_idx, column=0, sticky="w", pady=3, padx=8)
                if i == 0:
                    radio_button.invoke()
                row_idx += 1
        ...
        submit_button = ttk.Button(
            ...
            text="Submit Answer",
            style="ModernPrimary.TButton",
            command=self.submit_choice_answer,
        )
    ...
    def submit_choice_answer(self):
        try:
            if (
                self.current_question
                and "Select all that apply" in self.current_question["text"]
            ):
                selected_options = [
                    option
                    for option, var in self.checkbox_vars_dict.items()
                    if var.get()
                ]
                ...
                if not answer:
                    messagebox.showerror("No Selection", "Please select an answer.")
                    return
                print(
    ...
    def process_answer(self, answer):
        try:
            ...
            is_multi_select_question = self.current_question["ident"] in [
                "locations",
            ]

            if is_multi_select_question:
                individual_answers = [a.strip() for a in answer.split(",") if a.strip()]
                print(
    ...
    def show_diagnosis_results(self):
        ...
        if best_diagnosis:
            ...
            reasoning = best_diagnosis.get("reasoning", "No reasoning available")

            self.results_text.insert(tk.END, "🏥 DIAGNOSIS RESULTS\n")
            self.results_text.insert(tk.END, "=" * 50 + "\n\n")
            self.results_text.insert(tk.END, f"📋 Primary Diagnosis: {disease}\n")
            if self.show_confidence.get():
                self.results_text.insert(tk.END, f"    Confidence: {confidence:.1f}%\n")
            if self.detailed_reasoning.get():
                self.results_text.insert(tk.END, f"    Reasoning: {reasoning}\n")
    ...
    def show_no_diagnosis_message(self):
        ...
        self.results_text.insert(tk.END, "• Symptoms don't match known patterns\n")
        self.results_text.insert(tk.END, "• Additional information may be required\n")
        self.results_text.insert(
            tk.END, "• Condition may require professional examination\n\n"
        )
        self.results_text.insert(
            tk.END,
            "Please consult with a dermatologist or healthcare provider for proper evaluation.",
        )
        self.results_text.config(state="disabled")
        self.create_results_buttons()
        self.notebook.select(0)
    ...
    def create_results_buttons(self):
        for widget in self.results_frame.winfo_children():
            if isinstance(widget, tk.Frame) and any(
                isinstance(c, ttk.Button) for c in widget.winfo_children()
            ):
                widget.destroy()
        ...
        try:
            ...
            if os.name == "nt":
                ...
                messagebox.showwarning(
                    "Print",
                    "Printing not supported on this OS automatically. Report saved as HTML.",
                )
        except Exception as e:
...
{results_content}
    ...
    def save_settings(self):
        messagebox.showinfo(
            "Settings",
            "Settings saved successfully! (Functionality not fully implemented)",
        )
    ...
    def update_status(self, message, status_type="info"):
        try:
            ...
            self.status_label.config(text=f"● {message}", fg="white", bg=bg_color)
